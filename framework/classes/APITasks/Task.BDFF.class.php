...
class TaskBDFF extends AbstractTask
{
	...
	public function taskLive(){
		...
		$previous = (new Data())->store($store)->from(time()-(2*3600*$hours))->to(time()-(3600*$hours))->getAvg();
		...
		$previous_day = (new Data())->store($store)->from(time()-(2*3600*24))->to(time()-(3600*24))->getAvg();
